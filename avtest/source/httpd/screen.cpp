...
{
    std::vector<uint8_t>& outputBuffer = *(reinterpret_cast<std::vector<uint8_t>* >(png_get_io_ptr(png_ptr)));
...
extern "C" void cScreenCapture(unsigned char ** data, int * len) {
    ...
    png_set_write_fn (png_ptr_w, &out, &writeDataCallback, NULL);
